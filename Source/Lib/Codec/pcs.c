...
static void me_dctor(EbPtr p) {
    ...
    if (obj->vmaf_rdmult_scaling_factors)
        EB_FREE_ARRAY(obj->vmaf_rdmult_scaling_factors);
}
/*
...
static EbErrorType me_ctor(MotionEstimationData *object_ptr, EbPtr object_init_data_ptr) {
    ...
    if (init_data_ptr->enable_tpl_la) {
        ...
        if (init_data_ptr->static_config.tune == 3) {
            EB_MALLOC_ARRAY(object_ptr->vmaf_rdmult_scaling_factors,
                            adaptive_picture_width_in_mb * adaptive_picture_height_in_mb);
        } else {
            object_ptr->vmaf_rdmult_scaling_factors = NULL;
        }
