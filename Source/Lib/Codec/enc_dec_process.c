...
double aom_ssim2_new(const uint8_t *img1, int stride_img1, const uint8_t *img2, int stride_img2, int width,
                        int height) {
    int    i, j;
    int    samples    = 0;
    double ssim_total = 0;
...
double psnr_calculations_luma(Yv12BufferConfig blurred_temp, EbPictureBufferDesc *const source,int width,int height) {
    double mse_luma        = 0;
    double   psnr                = 0.0;
    uint64_t residual_distortion = 0;
    
    EbByte    input_buffer;
    EbByte    blurred_buffer;
    input_buffer   = &((source->buffer_y)[source->org_x + source->org_y * source->stride_y]);
    blurred_buffer = (&blurred_temp)->y_buffer;

    residual_distortion = 1;
    for (int row_index = 0; row_index < height; ++row_index) {
        for (int column_index = 0; column_index < width; ++column_index) {
            residual_distortion += (int64_t)SQR((int64_t)(input_buffer[column_index]) - (blurred_buffer[column_index]));
        }

        input_buffer += source->stride_y;
        blurred_buffer += (&blurred_temp)->y_stride;
    }
    mse_luma = residual_distortion/width/height;
    //fprintf(stderr,"mse=%f ",mse_luma);
    psnr     = 10 * log10(255 * 255 / mse_luma);
    return psnr;
}
