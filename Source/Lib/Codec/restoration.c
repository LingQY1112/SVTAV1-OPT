...
static void copy_and_extend_plane(uint8_t *src, int src_pitch, uint8_t *dst, int dst_pitch, int w, int h,
                                  int extend_top, int extend_left, int extend_bottom, int extend_right) {
    int i, linesize;
...
void svt_copy_and_extend_frame(EbPictureBufferDesc *src, Yv12BufferConfig *dst) {
    // Extend src frame in buffer
    // Altref filtering assumes 16 pixel extension
    const int et_y = 16;
    const int el_y = 16;
    int32_t   chroma_sub_log2[2] = {1, 1};//todo:send chroma sampling
    // Motion estimation may use src block variance with the block size up
    // to 64x64, so the right and bottom need to be extended to 64 multiple
    // or up to 16, whichever is greater.
    const int er_y = AOMMAX(src->width + 16, ALIGN_POWER_OF_TWO(src->width, 6)) - src->width;
    const int eb_y = AOMMAX(src->height + 16, ALIGN_POWER_OF_TWO(src->height, 6)) - src->height;
    const int uv_width_subsampling  = chroma_sub_log2[1];
    const int uv_height_subsampling = chroma_sub_log2[0];
    const int et_uv                 = et_y >> uv_height_subsampling;
    const int el_uv                 = el_y >> uv_width_subsampling;
    const int eb_uv                 = eb_y >> uv_height_subsampling;
    const int er_uv                 = er_y >> uv_width_subsampling;

    copy_and_extend_plane(src->buffer_y+src->org_y*src->stride_y+src->org_x,
                          src->stride_y,
                          dst->y_buffer,
                          dst->y_stride,
                          src->width,
    ...
    copy_and_extend_plane(src->buffer_cb + src->stride_cb * (src->org_y >> chroma_sub_log2[0]) + (src->org_x>>chroma_sub_log2[1]),
                          src->stride_cb,
                          dst->u_buffer,
                          dst->uv_stride,
        (src->width >> chroma_sub_log2[1]),
        (src->height >> chroma_sub_log2[0]),
                          et_uv,
                          el_uv,
                          eb_uv,
                          er_uv);

    copy_and_extend_plane(src->buffer_cr + src->stride_cr * (src->org_y >> chroma_sub_log2[0]) + (src->org_x >> chroma_sub_log2[1]),
                          src->stride_cr,
                          dst->v_buffer,
                          dst->uv_stride,
        (src->width >> chroma_sub_log2[1]),
