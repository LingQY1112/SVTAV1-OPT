...
EbErrorType svt_av1_verify_settings(SequenceControlSet *scs) {
    ...
    if (config->tune > 4) {
        SVT_ERROR(
            "Instance %u: Invalid tune flag [0 - 3, 0 for VQ, 1 for PSNR and 2 for SSIM and 3 for VMAF 4 for preeprocessing], your "
            "input: %d\n",
            channel_number + 1,
            config->tune);
        return_error = EB_ErrorBadParameter;
...
EbErrorType svt_av1_set_default_params(EbSvtAv1EncConfiguration *config_ptr) {
    ...
    config_ptr->vmaf_model_path=NULL;
    return return_error;
}
...
EB_API EbErrorType svt_av1_enc_parse_parameter(EbSvtAv1EncConfiguration *config_struct, const char *name,
    ...
    //*******************************************************************//
    if (!strcmp(name, "vmaf-model-path")) {
        static char temp_buffer[10];
        config_struct->vmaf_model_path = temp_buffer;
        fprintf(stderr,"PATH:  %s\n",config_struct->vmaf_model_path);
        strncpy(config_struct->vmaf_model_path, value, strlen(value));
